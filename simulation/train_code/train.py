...
def train(epoch, logger):
    ...
    for i in range(batch_num):
        gt_batch = shuffle_crop(train_set, opt.batch_size) # train_set: 242; batch_size: 5? 6?; gt_batch: 5/6 28 256 256
        gt = Variable(gt_batch).cuda().float()
        input_meas = init_meas(gt, mask3d_batch_train, opt.input_setting) # B 256 310
        optimizer.zero_grad()
