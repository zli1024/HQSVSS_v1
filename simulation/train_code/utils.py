...
def shuffle_crop(train_data, batch_size, crop_size=256, argument=True):
    if argument:
        ...
        processed_data = np.zeros((batch_size//2, crop_size, crop_size, 28), dtype=np.float32) # 2 256 256 28
        ...
        processed_data = torch.from_numpy(np.transpose(processed_data, (0, 3, 1, 2))).cuda().float() # 2 28 256 256
        ...
        for i in range(batch_size - batch_size // 2):
            ...
            gt_batch.append(arguement_2(gt_batch_2)) # 4 28 256 256
...
def gen_meas_torch(data_batch, mask3d_batch, Y2H=True, mul_mask=False):
    ...
    temp = shift(mask3d_batch * data_batch, 2) # B 28 256 310
    meas = torch.sum(temp, 1) # B 256 310
    if Y2H:
        meas = meas / nC * 2
        H = shift_back(meas)
        if mul_mask:
...
def init_mask(mask_path, mask_type, batch_size):
    mask3d_batch = generate_masks(mask_path, batch_size) # B 28 256 256
    if mask_type == 'Phi':
...
if __name__ == '__main__':
    ...
    mask3d_batch_train, input_mask_train = init_mask(mask_path, mask_type='Phi_PhiPhiT', batch_size=10)
    print(input_mask_train[0].shape, input_mask_train[1].shape)
