...
from mamba_ssm.ops.selective_scan_interface import selective_scan_fn # mamba_ssm: 1.2.0
...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, num_feat, squeeze_factor=16):
        ...
        self.attention = nn.Sequential(
            ...
            nn.Conv2d(num_feat, num_feat // squeeze_factor, 1, padding=0),  # TODO: 注意这里压缩的通道数能否被整除(其实这里用的是//)
...
class CAB(nn.Module):
    def __init__(self, num_feat, is_light_sr= False, compress_ratio=3,squeeze_factor=30):
        ...
        self.cab = nn.Sequential(
            nn.Conv2d(num_feat, num_feat // compress_ratio, 3, 1, 1),   # TODO: 注意这里压缩的通道数能否被整除(其实这里用的是//)
            nn.GELU(),
...
class VSSBlock(nn.Module):
    ...
    # NOTE：注意这里VSSBlock是论文中的RSSBlock
    # NOTE: 但是这里的RSSBlock中的VSSM模块只有2D-SSM(不符合论文中Fig.2(b)的结构)
    
    
class BasicLayer(nn.Module):
    """ The Basic MambaIR Layer in one Residual State Space Group
    Args:
        dim (int): Number of input channels. -> 28
    ...
    def forward(self, x, x_size):             #TODO: x:[B,L,C]   x_size:[256 310]
...
class MambaIR(nn.Module):
    r""" MambaIR Model
       ...
       Args:
           ...
           in_chans (int): Number of input image channels. Default: 29
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 3, 1)   # Extration的输入shape: [B 29 256 310] -> 输出:[B 96 85 103]
        ...
        for i_layer in range(self.num_layers): # 4层6-layers
            layer = ResidualGroup(
                dim=embed_dim,
                input_resolution=(patches_resolution[0], patches_resolution[1]),    #TODO: input_resolution如果没啥用就删了
        ...
        # for image denoising
        self.conv_last = nn.ConvTranspose2d(embed_dim, num_out_ch, 3, 3, 1)
    ...
    def forward_features(self, x):
        x_size = (x.shape[2], x.shape[3]) # 86 104
        x = self.patch_embed(x) # [B 96 86 104] -> [B,L,C]=[B,86*104,96]
    ...
    def forward(self, x):
        ...
        # Extraction
        x_first = self.conv_first(x) # x_first: [B 96 86 104]
        
        
        res = self.conv_after_body(self.forward_features(x_first)) + x_first # res: [B 96 86 104]
        x = x[:,:28,:,:] + self.conv_last(res) # x: [B 28 256 310]
...
class ResidualGroup(nn.Module):
    ...
    def forward(self, x, x_size): # [B L C] -> [B C H W] -> [B C H W] -> [B L C]
...
class PatchEmbed(nn.Module):
    ...
    Args:
        ...
        in_chans (int): Number of input image channels. Default: 28.
        embed_dim (int): Number of linear projection output channels. Default: 96.
        norm_layer (nn.Module, optional): Normalization layer. Default: None
    """

    def __init__(self, img_size=256, patch_size=32, in_chans=28, embed_dim=96, norm_layer=None):
        super().__init__()
    ...
    def forward(self, x):
        x = x.flatten(2).transpose(1, 2)  # b 256*310 96
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        x = x.transpose(1, 2).view(x.shape[0], self.embed_dim, x_size[0], x_size[1])  # [B,L,C] -> [B,C,H,W]
        return x
...
class HyPaNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.avg_pool(x)
        x = self.mlp(x) + 1e-6
        return x[:,:self.out_nc//2,:,:], x[:,self.out_nc//2:,:,:]  # [B, 4, 1, 1] * 2 (前半/后半)


class HQSVSS(nn.Module):
    def __init__(self, num_iterations=1):
        super(HQSVSS, self).__init__()
        ...
        for _ in range(num_iterations):
            self.denoisers.append(
                MambaIR(in_chans=29, out_chans=28, embed_dim=96, depths=(6, 6, 6, 6)),
            )
    ...
    def initial(self, y, Phi):
        ...
        y = y / nC * 2
        bs,row,col = y.shape
        y_shift = torch.zeros(bs, nC, row, col).cuda().float() # B 28 256 310
        for i in range(nC):
            y_shift[:, i, :, step * i:step * i + col - (nC - 1) * step] = y[:, :, step * i:step * i + col - (nC - 1) * step]
        z = self.fution(torch.cat([y_shift, Phi], dim=1)) # B 56 256 310 -> B 28 256 310
